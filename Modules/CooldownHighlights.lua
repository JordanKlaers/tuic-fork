...
local function UpdateHighlightFrame(trackerKey, slotIndex)
    ...
    -- Use Midnight Duration Object APIs
    -- =========================================================================
    local sourceIcon = slotInfo.icon
    local sourceCooldown = sourceIcon.Cooldown or sourceIcon.cooldown
    
    -- Get spellID for C_Spell API
    -- Check multiple possible properties (custom trackers use trackID/trackType)
    ...
    -- PRIORITY: Direct API calls for freshest data (no lag from source frame updates)
    -- Only fall back to source cooldown if we don't have a spellID
    
    -- Method 1: Try charges cooldown first (for spells with charges) - DIRECT API
    if spellID and C_Spell and C_Spell.GetSpellChargesCooldownDuration then
        local success = pcall(function()
            local chargeDuration = C_Spell.GetSpellChargesCooldownDuration(spellID)
            if chargeDuration then
                frame.cooldown:SetCooldownFromDurationObject(chargeDuration, true)
            end
        end)
        if success then cooldownApplied = true end
    end
    
    -- Method 2: C_Spell.GetSpellCooldownDuration (regular cooldowns) - DIRECT API
    if not cooldownApplied and spellID and C_Spell and C_Spell.GetSpellCooldownDuration then
        local success = pcall(function()
    ...
    -- Method 3: C_Spell.GetSpellCooldown traditional API - DIRECT API
    if not cooldownApplied and spellID and C_Spell and C_Spell.GetSpellCooldown then
        pcall(function()
    ...
    -- Method 4: Source cooldown pass-through (fallback for items/unknown spells)
    if not cooldownApplied and sourceCooldown then
    ...
    -- =========================================================================
    -- PURE PASS-THROUGH: Copy count/charge text from source icon
    -- Try Midnight API first, then fall back to FontString search
    -- NO conditionals on returned values - they may be secret
    -- spellID already obtained above for cooldown section
    -- =========================================================================
    
    -- Method 1: Try C_Spell.GetSpellDisplayCount (Midnight API - returns formatted string)
    ...
    -- Method 2: Try source icon's Count FontString (pass through directly)
    else
        ...
        -- Pass through directly - no conditionals on secret values
